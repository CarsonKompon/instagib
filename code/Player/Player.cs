...
public sealed class Player : Component
{
	...
	void UpdateCamera()
	{
		...
		if ( Scene.Camera is not null )
		{
			...
			Scene.Camera.WorldRotation = visualDirection;
	...
	void UpdateBounceReticle()
	{
		...
		var tr = Scene.Trace.Ray( Head.WorldPosition, Head.WorldPosition + dir.Forward * BounceRange )
		...
		if ( Network.IsOwner && bounceIndicator.IsValid() )
		{
			...
			if ( tr.Hit )
			{
				...
				bounceIndicator.WorldPosition = tr.HitPosition - visualDirection.Forward * 20f;
			}
		}
	}
	...
	public void Kill( Guid killer = default )
	{
		...
		Sound.Play( "snd-flesh-explode", WorldPosition );
		GameManager.Instance.SpawnGibs( WorldPosition + Vector3.Up * 32f, Color );
		if ( Network.IsOwner )
		{
	...
	void RotateBody()
	{
		...
		{
			Body.WorldRotation = Angles.Lerp( Body.WorldRotation, targetAngle, Time.Delta * 10f );
		}
	}
